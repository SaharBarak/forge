...
async function startNode({ dataDir: dataDir2 }) {
  storePath = path2.join(dataDir2, "store.jsonl");
  await fs2.mkdir(dataDir2, { recursive: true });
  try {
    const content = await fs2.readFile(storePath, "utf-8");
...
async function putDoc(doc) {
  ...
  docs.set(doc._id, doc);
  await fs2.appendFile(storePath, JSON.stringify(doc) + "\n", "utf-8");
...
async function deleteDoc(id) {
  requireStarted();
  docs.delete(id);
  await fs2.appendFile(storePath, JSON.stringify({ _id: id, _tombstone: true }) + "\n", "utf-8");
  return id;
}
...
var init_vector_index = __esm({
  "electron/connections/vector-index.js"() {
    ...
    loadIdMap = async () => {
      try {
        const raw = await fs3.readFile(mapPath, "utf-8");
    ...
    saveIdMap = async () => {
      await fs3.writeFile(mapPath, JSON.stringify(idMap), "utf-8");
    };
    load = async ({ dir, dims = 384 }) => {
      ...
      dataDir = dir;
      dimensions = dims;
      indexPath = path3.join(dir, "index.usearch");
      mapPath = path3.join(dir, "id-map.json");
...
var init_service = __esm({
  "electron/connections/service.js"() {
    ...
    startService = async ({ dataDir: dataDir2 }) => {
      serviceDataDir = path4.join(dataDir2, "connections");
...
var { bold: bold2, dim: dim2, italic: italic2, strikethrough: strikeStyle } = forgeTheme;
var HTML_ENTITY_MAP = {
  "&quot;": '"',
  "&apos;": "'",
  "&amp;": "&",
...
var getHighlightedSync = (code, lang) => {
  const key = `${lang}:${code}`;
...
var renderInline = (tokens) => tokens.map(renderInlineToken).join("");
...
var renderMarkdown = (markdown, _width) => {
  const tokens = Lexer.lex(markdown);
  return tokens.map((t) => renderToken(t, 0)).join("");
};

// src/lib/render/tool-call.ts
var TOOL_REGEX = /\[TOOL:\s*(\S+)\]\s*([\s\S]*?)\[\/TOOL\]/g;
var parseToolCalls = (content) => {
  const blocks = [];
  let lastIndex = 0;
...
var renderToolCall = (toolName, toolPrompt, status2 = "pending", resultSummary) => {
  const border = forgeTheme.border.accent;
...
var renderMessageWithToolCalls = (content, opts = {}) => {
...
var AUTH_FILE = path.join(FORGE_DIR, "auth.json");
var ensureDir = async () => {
...
var getStatus2 = () => ResultAsync.fromPromise(
...
var getStatus4 = () => ResultAsync.fromPromise(
  getService().then((s) => s.status()),
  (c) => statusFailed2("connections status failed", c)
);
var findSimilar = (text, k = 10, excludeId = null) => fromPromise(
...
var isContribution = (p) => !!p && typeof p === "object" && "kind" in p && "title" in p && p.kind !== "reaction";
...
var CLAUDE_CODE_PATH = path5.join(os2.homedir(), ".local", "bin", "claude");
...
var FileSystemAdapter = class {
  ...
  async readDir(dirPath) {
    try {
      const fullPath = path6.isAbsolute(dirPath) ? dirPath : path6.join(this.cwd, dirPath);
      const entries = await fs4.readdir(fullPath, { withFileTypes: true });
      return entries.map((entry) => ({
        name: entry.name,
        isDirectory: entry.isDirectory(),
        path: path6.join(fullPath, entry.name)
      }));
    } catch {
      return [];
    }
  ...
  async readFile(filePath) {
    try {
      const fullPath = path6.isAbsolute(filePath) ? filePath : path6.join(this.cwd, filePath);
  ...
  async writeFile(filePath, content) {
    try {
      const fullPath = path6.isAbsolute(filePath) ? filePath : path6.join(this.cwd, filePath);
      await this.ensureDir(path6.dirname(fullPath));
  ...
  async loadContext(contextDir) {
    const fullPath = path6.isAbsolute(contextDir) ? contextDir : path6.join(this.cwd, contextDir);
    const result = {
      brand: null,
      audience: null,
      research: [],
    ...
    const brandPath = path6.join(fullPath, "brand.md");
    result.brand = await this.readFile(brandPath);
    const audiencePath = path6.join(fullPath, "audience.md");
    ...
    for (const file of researchFiles) {
      ...
      if (content) {
        result.research.push({ file: path6.basename(file), content });
      }
    }
    ...
    for (const file of exampleFiles) {
      const content = await this.readFile(file);
      if (content) {
        result.examples.push({ file: path6.basename(file), content });
      }
    }
    ...
    for (const file of competitorFiles) {
      const content = await this.readFile(file);
      if (content) {
        result.competitors.push({ file: path6.basename(file), content });
      }
    }
    return result;
  }
  async readBrief(briefName) {
    ...
    const briefPath = path6.join(this.cwd, "briefs", withExt);
    return this.readFile(briefPath);
  }
  async listBriefs() {
    const briefsDir = path6.join(this.cwd, "briefs");
    const files = await this.glob("*.md", { cwd: briefsDir });
    return files.map((f) => path6.basename(f, ".md"));
  }
  ...
  async appendFile(filePath, content) {
    try {
      const fullPath = path6.isAbsolute(filePath) ? filePath : path6.join(this.cwd, filePath);
      await this.ensureDir(path6.dirname(fullPath));
      await fs4.appendFile(fullPath, content, "utf-8");
      return true;
    } catch {
      return false;
    }
...
var SessionPersistence = class {
  ...
  async initSession(session) {
    ...
    this.sessionDir = path7.join(this.config.outputDir, sessionName);
    ...
    await this.fs.writeFile(
      path7.join(this.sessionDir, "session.json"),
  ...
  async saveIncremental() {
    ...
    const jsonlPath = path7.join(this.sessionDir, "messages.jsonl");
  ...
  async generateTranscript() {
    ...
    await this.fs.writeFile(
      path7.join(this.sessionDir, "transcript.md"),
      lines.join("\n")
    );
  }
  /**
  ...
  async generateDraft() {
    ...
    await this.fs.writeFile(
      path7.join(this.sessionDir, "draft.md"),
      lines.join("\n")
    );
  }
  /**
  ...
  async saveSessionWithMemory(sessionWithMemory) {
    ...
    if (sessionWithMemory.memoryState) {
      await this.fs.writeFile(
        path7.join(this.sessionDir, "memory.json"),
...
async function introspectProject(opts) {
  ...
  try {
    const stat2 = await fs5.stat(projectDir);
  ...
  for (const file of allFiles) {
    const rel = path8.relative(projectDir, file).toLowerCase();
    let score = 0;
  ...
  if (scored.length < maxFiles) {
    ...
    for (const file of rest) {
      try {
        const stat2 = await fs5.stat(file);
        if (stat2.size > maxFileSize * 3) continue;
        const content = (await fs5.readFile(file, "utf-8")).toLowerCase();
        let contentScore = 0;
        for (const kw of keywords) {
          let idx = 0;
          let hits = 0;
  ...
  for (const { path: filePath } of topFiles) {
    try {
      ...
      snippets.push({
        path: path8.relative(projectDir, filePath),
        content
      });
    } catch {
    }
...
function createPersonasCommand() {
  ...
  personas.command("generate").description("Generate new personas for a domain").option("-d, --domain <domain>", "Domain or topic for the debate").option("-b, --brief <name>", "Generate from a brief file").option("-c, --count <n>", "Number of personas to generate", "5").option("-r, --roles <roles>", "Specific roles to include (comma-separated)").option("-n, --name <name>", "Name for the persona set", "custom").option("-o, --output <dir>", "Output directory", "personas").action(async (options) => {
    ...
    let contextPrompt = "";
    if (options.brief) {
      const briefPath = path9.join(cwd, "briefs", `${options.brief}.md`);
      try {
        const briefContent = await fs6.readFile(briefPath, "utf-8");
...
Generate exactly ${options.count} personas.`;
    ...
    try {
      ...
      const outputDir = path9.join(cwd, options.output);
      await fs6.mkdir(outputDir, { recursive: true });
      const personasFile = path9.join(outputDir, `${options.name}.json`);
      await fs6.writeFile(personasFile, JSON.stringify(personas2, null, 2));
      if (expertise) {
        const skillsFile = path9.join(outputDir, `${options.name}.skills.md`);
        await fs6.writeFile(skillsFile, expertise);
...
Use with: forge start --personas ${options.name}`);
  ...
  personas.command("list").description("List available persona sets").option("-d, --dir <dir>", "Personas directory", "personas").action(async (options) => {
    const cwd = process.cwd();
    const personasDir = path9.join(cwd, options.dir);
    try {
      const files = await fs6.readdir(personasDir);
      ...
      for (const file of jsonFiles) {
        ...
        const content = await fs6.readFile(path9.join(personasDir, file), "utf-8");
  ...
  personas.command("show <name>").description("Show details of a persona set").option("-d, --dir <dir>", "Personas directory", "personas").action(async (name, options) => {
    const cwd = process.cwd();
    const filePath = path9.join(cwd, options.dir, `${name}.json`);
    try {
...
function createExportCommand() {
  const exportCmd = new Command2("export").description("Export session data").option("-s, --session <dir>", "Session directory to export").option("-f, --format <fmt>", "Export format: md, json, html", "md").option("-t, --type <type>", "Export type: transcript, draft, summary, messages, all", "transcript").option("-o, --output <file>", "Output file path").option("-l, --latest", "Use the latest session", false).action(async (options) => {
    ...
    if (!sessionDir || options.latest) {
      const outputDir = path10.join(cwd, "output/sessions");
      try {
        const dirs = await fs7.readdir(outputDir, { withFileTypes: true });
        ...
        sessionDir = path10.join(outputDir, sessionDirs[0]);
...
`);
    ...
    const metadataPath = path10.join(sessionDir, "session.json");
    const messagesPath = path10.join(sessionDir, "messages.jsonl");
    let metadata = null;
    let messages = [];
    try {
      const metaContent = await fs7.readFile(metadataPath, "utf-8");
      metadata = JSON.parse(metaContent);
    } catch {
      console.warn("Warning: Could not load session metadata");
    }
    try {
      const msgContent = await fs7.readFile(messagesPath, "utf-8");
    ...
    switch (options.type) {
      ...
      case "all":
        ...
        for (const t of types) {
          ...
          const typeFile = options.output ? path10.join(path10.dirname(options.output), `${t}.${options.format}`) : path10.join(sessionDir, `export-${t}.${options.format}`);
          await fs7.writeFile(typeFile, typeOutput);
    ...
    const outputPath = options.output || path10.join(sessionDir, `export-${filename}`);
    await fs7.writeFile(outputPath, output);
...
async function runBatch(pattern, options) {
  ...
  const outputDir = path11.resolve(cwd, options.output || "output/batch");
...
\u{1F525} Forge Batch Processing`);
  ...
  if (options.resume) {
    ...
    toProcess = briefPaths.filter((p) => !processed.has(path11.basename(p, ".md")));
...
async function processBrief(briefPath, opts) {
  const briefName = path11.basename(briefPath, ".md");
  const startTime = Date.now();
  if (!opts.json) {
  ...
  try {
    ...
    const briefContent = await fs8.readFile(briefPath, "utf-8");
    ...
    const config = {
      ...
      contextDir: path11.join(opts.cwd, "context"),
    ...
    const persistence = new SessionPersistence(fsAdapter, {
      outputDir: path11.join(opts.outputDir, briefName)
    });
...
async function getProcessedBriefs(outputDir) {
  ...
  try {
    const entries = await fs8.readdir(outputDir, { withFileTypes: true });
    for (const entry of entries) {
      if (entry.isDirectory() && !entry.name.startsWith(".")) {
        try {
          await fs8.access(path11.join(outputDir, entry.name, "session.json"));
...
async function listSessions(options) {
  const cwd = process.cwd();
  const sessionsDir = path12.resolve(cwd, options.output || "output/sessions");
  try {
    const entries = await fs9.readdir(sessionsDir, { withFileTypes: true });
    ...
    for (const dir of sessionDirs) {
      const meta = await loadSessionMeta(path12.join(sessionsDir, dir.name));
      if (meta) {
        sessions.push(meta);
      }
    }
...
async function showSession(name, options) {
  ...
  try {
    const sessionPath = path12.join(sessionDir, "session.json");
    const sessionData = JSON.parse(await fs9.readFile(sessionPath, "utf-8"));
    let messages = [];
    try {
      const messagesPath = path12.join(sessionDir, "messages.jsonl");
      const content = await fs9.readFile(messagesPath, "utf-8");
...
async function deleteSession(name, options) {
  ...
  if (!options.force) {
    ...
    const confirmed = await new Promise((resolve4) => {
      rl.question(`Delete session "${path12.basename(sessionDir)}"? [y/N]: `, (answer) => {
        rl.close();
  ...
  try {
    await fs9.rm(sessionDir, { recursive: true });
    console.log(chalk.green(`Deleted: ${path12.basename(sessionDir)}`));
  } catch (error) {
...
async function exportSession(name, options) {
  ...
  try {
    ...
    try {
      const messagesPath = path12.join(sessionDir, "messages.jsonl");
      const content2 = await fs9.readFile(messagesPath, "utf-8");
    ...
    const destPath = options.dest || path12.join(sessionDir, `export.${ext}`);
    await fs9.writeFile(destPath, content);
...
async function cleanSessions(options) {
  ...
  try {
    ...
    const toDelete = [];
    for (const dir of sessionDirs) {
      const dirPath = path12.join(sessionsDir, dir.name);
    ...
    for (const name of toDelete) {
      await fs9.rm(path12.join(sessionsDir, name), { recursive: true });
...
async function loadSessionMeta(sessionDir) {
  try {
    const sessionPath = path12.join(sessionDir, "session.json");
    const data = JSON.parse(await fs9.readFile(sessionPath, "utf-8"));
    let messageCount = 0;
    try {
      const messagesPath = path12.join(sessionDir, "messages.jsonl");
      const content = await fs9.readFile(messagesPath, "utf-8");
    ...
    return {
      id: data.id,
      projectName: data.projectName || data.config?.projectName || path12.basename(sessionDir),
...
async function findSession(sessionsDir, nameOrIndex) {
  try {
    ...
    if (!isNaN(index2) && index2 >= 1 && index2 <= sessionDirs.length) {
      return path12.join(sessionsDir, sessionDirs[index2 - 1]);
    }
    const match = sessionDirs.find((d) => d.includes(nameOrIndex));
    if (match) {
      return path12.join(sessionsDir, match);
    }
    return null;
  } catch {
    return null;
...
async function runWatch(options) {
  const cwd = process.cwd();
  const contextDir = path13.resolve(cwd, options.context || "context");
  const debounceMs = parseInt(options.debounce || "1000", 10);
  if (options.brief) {
    try {
      await fs10.access(options.brief);
    } catch {
  ...
  const watchRecursive = async (dir) => {
    const watcher = watch(dir, { recursive: true }, (eventType, filename) => {
      if (filename && !filename.startsWith(".")) {
        handleChange(path13.join(dir, filename), eventType);
      }
    });
    return watcher;
  };
...
function getGlobalConfigPath() {
  ...
  return path14.join(home, CONFIG_FILENAME);
}
function getLocalConfigPath() {
  return path14.join(process.cwd(), CONFIG_FILENAME);
}
      ...
      const resolvedId = allResult.match(
        (docs2) => {
          const match = docs2.find(
            (d) => isContribution2(d.payload) && d.id.startsWith(idOrPrefix)
          );
...
async function generatePersonasForGoal(cwd, goal, projectName) {
  ...
  try {
    ...
    const personasDir = path16.join(cwd, "personas");
    await fs12.mkdir(personasDir, { recursive: true });
    await fs12.writeFile(
      path16.join(personasDir, `${safeName}.json`),
      JSON.stringify(personas, null, 2)
    );
    if (skills) {
      await fs12.writeFile(
        path16.join(personasDir, `${safeName}.skills.md`),
        skills
      );
    }
...
async function selectAgentsInteractively(personas) {
  ...
  const showMenu = () => {
    ...
    personas.forEach((agent, index2) => {
      ...
      console.log(`  ${checkbox} ${index2 + 1}. ${name} (${agent.nameHe}) - ${agent.role}`);
    });
    console.log("");
...
async function selectPersonas(cwd, goal, projectName) {
  ...
  let files = [];
  try {
    const allFiles = await fs12.readdir(personasDir);
    files = allFiles.filter((f) => f.endsWith(".json") && !f.includes(".skills"));
  } catch {
  }
  const options = [];
  for (const file of files) {
    const name = path16.basename(file, ".json");
    try {
      const content = await fs12.readFile(path16.join(personasDir, file), "utf-8");
    ...
    rl.question("Select option [G]: ", async (answer) => {
      ...
      if (a === "g" || a === "") {
        ...
        if (selection > 0 && selection <= options.length) {
          ...
          let skills;
          try {
            skills = await fs12.readFile(
              path16.join(personasDir, `${selected.name}.skills.md`),
              "utf-8"
            );
          } catch {
          }
...
\u{1F4CB} Using persona set: ${personaSetName}`);
  ...
  if (personaSetName && availablePersonas === AGENT_PERSONAS) {
    const personasPath = path16.join(cwd, "personas", `${personaSetName}.json`);
    try {
  ...
  if (personaSetName) {
    const skillsPath = path16.join(cwd, "personas", `${personaSetName}.skills.md`);
    try {
...
\u{1F525} Starting Forge: ${projectName}`);
  ...
  console.log(`\u{1F4DC} Live transcript: ${persistence.getSessionDir()}/transcript.md`);
  console.log(`\u{1F4E6} Raw events:      ${persistence.getSessionDir()}/messages.jsonl
`);
...
program.action(async () => {
  ...
  let intent = { kind: "exit" };
  const { HomeScreen: HomeScreen2 } = await Promise.resolve().then(() => (init_HomeScreen(), HomeScreen_exports));
  const homeApp = render(
    React6.createElement(HomeScreen2, {
      did,
      sessionCount,
      onStartNew: () => {
        intent = { kind: "start" };
  ...
  if (intent.kind === "start") {
    console.log(`
${CYAN}Launching deliberation session\u2026${RESET2}
`);
    await program.parseAsync(["node", "forge", "start"]);
  } else if (intent.kind === "load") {
    console.log(`
${CYAN}Loading session: ${intent.name}${RESET2}
`);
