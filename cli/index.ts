...
// Default action - launch interactive Ink TUI
program.action(async () => {
  ...
  console.log(`${CYAN}${BOLD}║${RESET}  ${DIM}Decentralized · Keyless · Open Source${RESET}                ${CYAN}${BOLD}║${RESET}`);
  console.log(`${CYAN}${BOLD}╚══════════════════════════════════════════════════════╝${RESET}`);
  console.log(`  ${DIM}${formatQuote(getRandomQuote())}${RESET}`);
  console.log('');
  ...
  if (!authState) {
    ...
    createResult.match(
      (s) => console.log(`${GREEN}  ✔ Identity: ${s.did.slice(0, 24)}…${RESET}`),
      (err) => console.log(`${YELLOW}  ⚠ ${err.message}${RESET}`)
    );
  } else {
    console.log(`${GREEN}  ✔ Identity: ${authState.did.slice(0, 24)}…${RESET}`);
  }

  console.log(`\n  Run ${GREEN}forge start${RESET} to begin a deliberation session.`);
  console.log(`  Run ${GREEN}forge community list${RESET} to browse peer contributions.`);
  console.log(`  Run ${GREEN}forge --help${RESET} for all commands.\n`);
});
// ---- graceful shutdown ----
