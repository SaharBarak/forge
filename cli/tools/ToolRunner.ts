/**
 * ToolRunner — Manages available tools and dispatches tool calls during
 * sessions. Now permission-aware: destructive tools request user approval
 * before execution via the injected PermissionBroker.
 */

import * as path from 'path';
import { randomUUID } from 'crypto';
import { GeminiTool } from './GeminiTool';
import type { PermissionBroker, PermissionRequest } from '../../src/lib/render/permission';
import { classifyToolRisk } from '../../src/lib/render/permission';

export interface ToolResult {
  success: boolean;
  outputPath?: string;
  description?: string;
  error?: string;
  denied?: boolean;
}

interface SessionTools {
  gemini?: GeminiTool;
}

export class ToolRunner {
  private tools: SessionTools = {};
  private counter = 0;
  private permissionBroker?: PermissionBroker;

  constructor(permissionBroker?: PermissionBroker) {
    this.permissionBroker = permissionBroker;
  }

  enableGemini(apiKey: string): void {
    this.tools.gemini = new GeminiTool(apiKey);
  }

  setPermissionBroker(broker: PermissionBroker): void {
    this.permissionBroker = broker;
  }

  getAvailableTools(): string[] {
    const available: string[] = [];
    if (this.tools.gemini) {
      available.push('image-generation', 'graph-generation');
    }
    return available;
  }

  async runTool(name: string, args: Record<string, string>, outputDir: string): Promise<ToolResult> {
    this.counter++;
    const timestamp = Date.now();

    // Permission gating — ask broker before running non-safe tools.
    const risk = classifyToolRisk(name);
    if (this.permissionBroker && risk !== 'safe') {
      const request: PermissionRequest = {
        id: randomUUID(),
        toolName: name,
        toolPrompt: args.prompt || args.description || '',
        risk,
        timestamp: new Date(),
      };
      const decision = await this.permissionBroker.requestPermission(request);
      if (decision === 'deny') {
        return { success: false, error: 'User denied tool execution', denied: true };
      }
    }

    try {
      switch (name) {
        case 'image-generation': {
          if (!this.tools.gemini) return { success: false, error: 'Gemini not configured' };
          const outputPath = path.join(outputDir, `image-${timestamp}-${this.counter}.png`);
          const description = await this.tools.gemini.generateImage(
            args.prompt || args.description || 'Generate an image',
            outputPath
          );
          return { success: true, outputPath, description };
        }

        case 'graph-generation': {
          if (!this.tools.gemini) return { success: false, error: 'Gemini not configured' };
          const outputPath = path.join(outputDir, `graph-${timestamp}-${this.counter}.png`);
          const description = await this.tools.gemini.generateGraph(
            args.data || '',
            args.description || 'Generate a chart',
            outputPath
          );
          return { success: true, outputPath, description };
        }

        default:
          return { success: false, error: `Unknown tool: ${name}` };
      }
    } catch (error: any) {
      return { success: false, error: error.message };
    }
  }
}
