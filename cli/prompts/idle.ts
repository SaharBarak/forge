...
export async function runIdleMode() {
  ...
  while (true) {
    ...
    const action = await p.select({
      message: `What would you like to do?${configLabel}`,
      options: [
...
async function handleApiTest() {
  ...
  try {
    ...
    p.log.info('Make sure ANTHROPIC_API_KEY is set or use "Set API key".');
  }
}
...
async function handleTokenFlow(state: IdleState) {
  if (state.apiKey) {
    p.log.info(`Current API key: ${state.apiKey.slice(0, 10)}...`);
  }
...
function showHelpNote() {
  const help = [
    ...
    `${chalk.green('Set API key')}     - Configure your Anthropic API key`,
    '',
